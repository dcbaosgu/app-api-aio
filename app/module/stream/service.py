...
stream_crud = BaseCRUD("stream-video", engine_aio)
...
class StreamService:
    ...
    async def update(self, stream_id: str, data: dict):
        result = await self.stream_crud.update_by_id(stream_id, data)
        if not result: 
            raise ErrorCode.InvalidStreamId()
        return result
    
    async def get(self, stream_id):
        result = await self.stream_crud.get_by_id(stream_id)
        if not result: 
            raise ErrorCode.InvalidStreamId()
        return result

    async def delete(self, stream_id):
        result = await self.stream_crud.delete_by_id(stream_id)
        if not result: 
            raise ErrorCode.InvalidStreamId()
        return result
    ...
    async def play_list(self, stream_id: str):
        ...
        abs_path = Path("/opt/python-project/app") / master_path.lstrip("/")
    ...
    async def play_master(self, stream_id: str, resolution: str = "auto"):
        ...
        master_path = stream.get("path")
        abs_path = Path("/opt/python-project/app") / master_path.lstrip("/")
        # abs_path = Path("/opt/python-project/app/asset/stream/dance1/master.m3u8")
        if not abs_path.exists():
            raise ErrorCode.M3U8FileNotFound()
...
steam_service = StreamService(stream_crud)