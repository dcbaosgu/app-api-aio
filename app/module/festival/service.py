...
event_crud = BaseCRUD("fes-event", engine_aio)
ticket_crud = BaseCRUD("fes-ticket", engine_aio)
...
class EventService:
    ...
    async def get(self, event_id: str):  
        result = await self.event_crud.get_by_id(event_id)
        if not result:
            raise ErrorCode.EventNotFound()
        return result

    async def update(self, event_id: str, data: dict):
        result = await self.event_crud.update_by_id(event_id, data)
        if not result:
            raise ErrorCode.EventNotFound()
        return result

    async def delete(self, event_id: str):
        result = await self.event_crud.delete_by_id(event_id)
        if not result:
            raise ErrorCode.EventNotFound()
        return result
...
event_service = EventService(event_crud)
ticket_service = TicketService(ticket_crud=ticket_crud, event_crud=event_crud)