...
@router.post("/add/{user_id}", status_code=201, responses={
                201: {"model": schema.CartResponse, "description": "Add items success"}})
async def add_cart(user_id: str, data: schema.AddCart):
    result = await controller.add_cart(user_id, data)
    return result


@router.put("/edit/{user_id}", status_code=200, responses={
                200: {"model": schema.CartResponse, "description": "Edit items success"}})
async def edit_cart(user_id: str, data: schema.EditCart):
    result = await controller.edit_cart(user_id, data)
    return result


@router.delete("/delete/{user_id}", status_code=200, responses={
                200: {"model": schema.CartResponse, "description": "Delete items success"}})
async def delete_cart(user_id: str, data: schema.DeleteCart):
    result = await controller.delete_cart(user_id, data)
    return result


@router.get("/get/{user_id}", status_code=200, responses={
                200: {"model": schema.CartResponse, "description": "Get items success"}})
