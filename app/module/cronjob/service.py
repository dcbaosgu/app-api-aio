...
cron_crud = BaseCRUD("cron-job", engine_aio)
...
class CronService:
    ...
    async def update(self, cron_id: str, data: dict):
        result = await self.crud.update_by_id(cron_id, data)
        if not result:
    ...
    async def getdb(self, cron_id):
        result = await self.crud.get_by_id(cron_id)
        if not result:
            raise ErrorCode.InvalidCronId()
        return result
    ...
    async def delete(self, cron_id):
        result = await self.crud.delete_by_id(cron_id)
        if not result:
            raise ErrorCode.InvalidCronId()
...
cron_service = CronService(cron_crud)