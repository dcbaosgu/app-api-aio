...
from .controller import EventController, TicketController
from . import schema
...
@router.post("/event/create", status_code=201, responses={
                201: {"model": schema.EventResponse, "description": "Create items success"}})
async def create_event(data: schema.EventCreate):
    result = await event_controller.create(data.model_dump())
    return result


@router.get("/event/get/{event_id}", status_code=200, responses={
                200: {"model": schema.EventResponse, "description": "Get items success"}})
...
@router.put("/event/edit/{event_id}", status_code=200, responses={
                200: {"model": schema.EventResponse, "description": "Edit items success"}})
async def update_event(event_id: str, data: schema.EventUpdate):
...
@router.get("/event/search", status_code=200, responses={
                200: {"model": schema.PaginatedEventResponse, "description": "Get items success"}})
...
@router.post("/ticket/check-out", status_code=201, responses={
                201: {"model": schema.TicketResponse, "description": "Create items success"}})
async def checkout_tickets(data: schema.CheckoutRequest):
    result = await ticket_controller.checkout(data.model_dump())
    return result


@router.post("/ticket/check-in", status_code=200, responses={
                201: {"model": schema.TicketResponse, "description": "Edit items success"}})
async def checkin_ticket(data: schema.CheckinRequest):
...
@router.get("/ticket/search", status_code=200, responses={
                200: {"model": schema.PaginatedTicketResponse, "description": "Get items success"}})
